...
class UserService:
    ...
    def create_user(self, user_in: UserCreate, role_id: uuid.UUID | None = None) -> UserModel:
        if role_id is None:
    ...
    def list_users(self, offset: int, limit: int) -> tuple[list[UserModel], int]:
    ...
    def get_user(self, user_id: uuid.UUID) -> UserModel:
        user_db = self.db.get(UserModel, user_id)
        if user_db is None:
    ...
    def verify_refresh_token(self, raw_token: str) -> UserModel | None:
